...
| R-011 | `/metrics` queda expuesto sin autorización                       | Media | Alto    | Loopback local y Bearer técnico obligatorio en producción               | Mitigado |
| R-012 | Sin trazas distribuidas ni alertas conectadas                    | Media | Medio   | W3C/OTLP, Collector, Alertmanager y gate runtime E0-H3B                 | Mitigado |
...
| R-018 | UI futura almacena Bearer token de forma insegura                | Media | Alto    | BFF y cookies HttpOnly; pruebas prohíben token en body/storage          | Mitigado |
...
| R-022 | Tokens Shopify pueden filtrarse o quedar sin rotar               | Media | Crítico | AES-GCM/AAD, keyring, rotación, errores redactados y mock               | Mitigado |
...
| R-026 | Rotar el secreto webhook invalida entregas en tránsito           | Media | Alto    | Activo+anterior cifrados con deadline configurable y prueba contractual | Mitigado |
...
| R-030 | PII real queda retenida sin política aprobada                    | Media | Crítico | Runtime listo, pero tráfico live bloqueado hasta resolver DP-003        | Abierto  |
...
| R-055 | Texto inbound queda retenido después de su plazo                 | Media | Crítico | Purga irreversible por deadline, lote/lock, auditoría y trigger DB      | Mitigado |
...
| R-060 | Claims concurrentes sobrescriben el agente actual                | Alta  | Alto    | Lock, versión esperada, idempotencia e historial único por versión      | Mitigado |
| R-061 | Membresía revocada permanece asignada a una conversación         | Media | Alto    | Revocación transaccional, lock tenant, historial, outbox y prueba race  | Mitigado |
| R-062 | Telemetría caída degrada o tumba requests de negocio             | Media | Alto    | Timeout, flags, kill switch, errores absorbidos y prueba de caída       | Mitigado |
| R-063 | Spans o alertas filtran PII, secretos o alta cardinalidad        | Media | Crítico | Atributos enumerados, receptor mínimo y pruebas negativas               | Mitigado |
| R-064 | Reinicio pierde baseline de transición de alertas                | Media | Medio   | Hidratación Alertmanager v2, parser cerrado, retry y drill de reinicio  | Mitigado |
...
| R-069 | CSRF o fijación de sesión permite rotar/revocar otro contexto    | Media | Alto    | Origin exacto, CSRF CSPRNG, SameSite y rotación atómica                 | Mitigado |
| R-070 | UI confía en tenant/IDs del navegador o filtra datos sensibles   | Media | Crítico | Tenant desde principal, doble validación y proyección BFF mínima        | Mitigado |
| R-071 | Evaluadores concurrentes crean alertas abiertas duplicadas       | Alta  | Alto    | Lock tenant transaccional e índice parcial único                        | Mitigado |
| R-072 | Evaluación de alertas ejecuta scans o consultas por recurso      | Media | Alto    | Ventana/lote validados y un agregado SQL por lote                       | Mitigado |
| R-073 | Alerta copia PII, IDs fuente o cardinalidad no acotada           | Media | Crítico | Tabla/proyección mínima, etiquetas cerradas y pruebas negativas         | Mitigado |
| R-074 | Dump local temporal expone datos o queda abandonado              | Media | Crítico | Directorio ignorado, modo 0600, cleanup y comprobación de ausencia      | Mitigado |
| R-075 | Restore local se interpreta como continuidad productiva          | Media | Alto    | Alcance explícito; exigir offsite cifrado y RPO/RTO aprobados           | Abierto  |
| R-076 | Serializable crea conflictos entre pedidos independientes        | Alta  | Alto    | Lock por agregado, READ COMMITTED, retry y gate de 500 pedidos          | Mitigado |
| R-077 | Carga simulada se extrapola a capacidad productiva               | Media | Crítico | Reporte local explícito; repetir en infra/proveedores objetivo          | Abierto  |
| R-078 | Escáner propio omite un formato secreto o produce falso negativo | Media | Crítico | Patrones high-confidence+self-test; añadir scanner especializado        | Abierto  |
| R-079 | Tags CI o CSP inline amplían supply-chain/XSS residual           | Media | Alto    | Pinear SHA y adoptar nonce/hash CSP antes de producción                 | Abierto  |
| R-080 | Smoke fallido deja procesos o listeners huérfanos                | Media | Alto    | Procesos directos, `finally`, SIGTERM y prueba activa de puertos        | Mitigado |
| R-081 | Rollback de código resulta incompatible con esquema expandido    | Media | Crítico | Compatibilidad expand/contract; hotfix forward y nunca `down` en crisis | Abierto  |
| R-082 | Presupuestos del drill local se interpretan como SLO productivos | Media | Alto    | Alcance/reportes explícitos; SLO requiere telemetría y aprobación real  | Abierto  |
| R-083 | Purga anticipada o concurrente elimina contenido vigente         | Media | Crítico | Deadline+trigger DB, advisory lock, SKIP LOCKED y pruebas negativas     | Mitigado |
| R-084 | Imports nuevos erosionan fronteras y crean ciclos entre módulos  | Media | Alto    | Gate fail-closed, allowlist exacta y fixtures permitidos/prohibidos     | Mitigado |
| R-085 | Cartera simulada se interpreta como recaudo o pierde precisión   | Media | Alto    | Semántica explícita y BIGINT como decimal string con prueba >MAX_SAFE   | Mitigado |
